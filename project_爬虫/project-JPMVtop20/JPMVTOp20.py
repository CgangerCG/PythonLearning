...
def write_in_file(info):
    with open('JapanMVTop20.csv', 'a', encoding='GBK') as f:
...
def get_more_pages():
    url = 'http://vchart.yinyuetai.com/vchart/v?area=JP&date={}'
    d1 = datetime.datetime.strptime('20140630', '%Y%m%d')
    d2 = datetime.datetime.strptime('20180903', '%Y%m%d')
    delta = datetime.timedelta(days=7)
    while d1 <= d2:
        i = d1.strftime('%Y%m%d')
        get_one_page(url.format(i),d1)
        d1 = d1 + delta
        print(d1)
    #get_one_page(url)
